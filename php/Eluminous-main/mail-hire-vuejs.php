...
if(isset($_REQUEST))
{
	...
	$sql_statement='INSERT INTO tbl_campaign_data (camp_id, name, email, phone_number, skype_id,message, IP_address, entry_date,`page_url`) VALUES ("2", "'.$name .'", "'.$email .'", "'.$number .'", "'.$companyName .'", "'.$comment .'", "'.$IP_address .'", "'.date('Y-m-d H:i:s').'", "'.$page_url.'");';
	$result = mysqli_query($conn,$sql_statement);
	/*-------------------------DATABASE END--------------------------*/
	$_SESSION['name'] = $name ;
	$message = '';
		$subject = 'New Enquiry: Hire vuejs Developers';	// Your Email Subject.

		$message .= '<span style="display:block;color:#000;"> Hi Admin, </span>';
		$message .= '<span style="display:block;color:#000;">Received New Enquiry from eLuminous Technologies for Hire vuejs Developers. Please have a look on user detail.Below is the user detail </span>';
		...
		$subject = 'Thank you! We have received your request for Hire vuejs Developers';	
		$skype_account = "eluminoustechnologies?chat";
		$mailSubject .='<tr><td><span style="font-size:16px;line-height:20px;color:#333;font-family:Segoe UI,Tahoma">Hello '.$name.',<br> <br>Thank you! We have received your request for Hire vuejs Developers.<br> <br>eLuminous Technologies success relies on the success of our customers. Following the latest technology trends, we help them in driving more agile process with our proven IT Solutions.<br><br>One of our representative will get in touch with you shortly to get an overview of your project ideas & offer solutions tailored for your needs.<br><br>We look forward providing top-notch quality service to you. Thank you once again for your time & consideration.<br><br>Best Regards,<br>Team eLuminous Technologies</b><br><br></span></td></tr>';
