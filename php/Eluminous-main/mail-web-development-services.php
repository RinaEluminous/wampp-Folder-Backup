...
if(isset($_REQUEST))
{
	...
	$sql_statement='INSERT INTO tbl_campaign_data (camp_id, name, email, phone_number, skype_id,message, IP_address, entry_date,`page_url`) VALUES ("1", "'.$name .'", "'.$email .'", "'.$number .'", "'.$companyName .'", "'.$comment .'", "'.$IP_address .'", "'.date('Y-m-d H:i:s').'", "'.$page_url.'");';
	$result = mysqli_query($conn,$sql_statement);
	/*-------------------------DATABASE END--------------------------*/
	$_SESSION['name'] = $name ;
	$message = '';
		$subject = 'New Enquiry: Adwords New Enquiry';	// Your Email Subject.
